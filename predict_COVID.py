...
class results_per_class_struct(NamedTuple):
    accuracy: list
    recall: list
    precision: list
    f1: list
...
model_names = ["L6N_ALL_DIST_ORIG_ENC-100_FOLD-01_of_05_0.1545",
    ...
    "L6N_ALL_DIST_ORIG_ENC-100_FOLD-05_of_05_0.1488"] #models de enfoque distintivo

model_names2 = ["L6N_ALL_AGLO_ORIG_ENC-100_FOLD-01_of_05_0.1568.model",
    "L6N_ALL_AGLO_ORIG_ENC-100_FOLD-02_of_05_0.1561.model",
    "L6N_ALL_AGLO_ORIG_ENC-100_FOLD-03_of_05_0.1591.model",
    "L6N_ALL_AGLO_ORIG_ENC-100_FOLD-04_of_05_0.1529.model",
    "L6N_ALL_AGLO_ORIG_ENC-100_FOLD-05_of_05_0.1642.model"] #models de enfoque aglomerativo
...
def predict_COVID(task=15, model_name="FFM_demo_0.7606", dataset_key="", label_set=[]):
...
def GetConfusionMatrix(test_data, classes):
    ...
    num_items_per_class = [0 for x in range(num_classes)] # suma por filas
    num_preds_per_class = [0 for x in range(num_classes)] #suma por columnas
    num_correct_per_class = [0 for x in range(num_classes)]
    accuracy_per_class = [0.0 for x in range(num_classes)]
    recall_per_class = [0.0 for x in range(num_classes)]
    precision_per_class = [0.0 for x in range(num_classes)]
    f1_measure_per_class = [0.0 for x in range(num_classes)]
    ...
    for item in test_data:
        ...
        num_preds_per_class[prediction_coord] += 1
        ...
        if label_coord == prediction_coord:
            ...
            num_correct += 1
            num_correct_per_class[label_coord] += 1
        else:
    ...
    for item in range(num_classes):
        if num_items_per_class[item] > 0:
            ...
            accuracy_per_class[item] = num_correct_per_class[item] / num_items_per_class[item]

            recall_per_class[item] = matrix[item][item] / num_items_per_class[item]
            precision_per_class[item] = matrix[item][item] / num_preds_per_class[item]
            if recall_per_class[item]==0.0 and precision_per_class[item]==0.0:
                f1_measure_per_class[item] = 0.0
            else:
                f1_measure_per_class[item] = 2 * (recall_per_class[item] * precision_per_class[item])/(recall_per_class[item] + precision_per_class[item])
        else:
            avg_length_per_class[item] = 0
            accuracy_per_class[item] = 0.0
            recall_per_class[item] = 0.0
            precision_per_class[item] = 0.0
            f1_measure_per_class[item] = 0.0

    results_per_class = results_per_class_struct(accuracy_per_class, recall_per_class, precision_per_class, f1_measure_per_class)
    if num_correct > 0:
    ...
    return matrix, statistics, error_matrix, results_per_class

def PrintStatistics(filename, statistics, classes, results_per_class):
    # class confusion_matrix_struct(NamedTuple):
    ...
    with open(filename, 'w') as f:
        num_classes = len(classes)
        print('\n[TEST STATISTICS]\n')
        print('\t[#][CLASS][# SAMPLES][AVG LENGTH][ACCURACY][RECALL][PRECISION][F1]\n')

        f.write('\n[TEST STATISTICS]\n')
        f.write('#\tCLASS\t# SAMPLES\tAVG LENGTH\tACCURACY\tRECALL\tPRECISION\tF1\n')

        for i in range(num_classes):
            print('\t[%d][%s][%d][%2.2f][%2.2f][%2.2f][%2.2f][%2.2f]' % (i, classes[i],
                                                                         statistics[keys['num_items_per_class']][i],
                                                                         statistics[keys['avg_length_per_class']][i],
                                                                         results_per_class[keys_per_class['accuracy']][i],
                                                                         results_per_class[keys_per_class['recall']][i],
                                                                         results_per_class[keys_per_class['precision']][i],
                                                                         results_per_class[keys_per_class['f1']][i]))
            f.write('%d\t%s\t%d\t%2.2f\t%2.2f\t%2.2f\t%2.2f\t%2.2f\n' % (i, classes[i],
                                                                         statistics[keys['num_items_per_class']][i],
                                                                         statistics[keys['avg_length_per_class']][i],
                                                                         results_per_class[keys_per_class['accuracy']][i],
                                                                         results_per_class[keys_per_class['recall']][i],
...
matrix, statistics, error_matrix, results_per_class = GetConfusionMatrix(data, label_set)

statistics_filename = os.path.join(PREDICTIONS_DIR, "{}_statistics.txt".format(name))
PrintStatistics(statistics_filename, statistics, label_set, results_per_class)
